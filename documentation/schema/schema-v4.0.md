# Stream Forge Database Schema v4.0

**Version:** 4.0  
**Date:** June 20, 2026  
**Status:** Updated - Phase 15 Transcription Job Metadata and Caption Delivery Alignment
...
| StoragePath | VARCHAR(1000) | NOT NULL | Canonical caption/transcription file path |
| Status | ENUM | NOT NULL | Pending, Processing, Completed, Failed |
| CorrelationId | VARCHAR(200) | NULL | Application correlation identifier shared across related format rows |
| WorkerJobId | VARCHAR(200) | NULL | Worker-side transcription job identifier |
| Source | VARCHAR(50) | NOT NULL | Provider/runtime source (local-faster-whisper, OpenAI, Azure, Manual) |
| Model | VARCHAR(200) | NULL | Requested or resolved provider model name |
| FailureReason | VARCHAR(2000) | NULL | Last reported failure reason |
| CreatedAt | TIMESTAMP | NOT NULL | Creation timestamp |
...
- `IX_VideoTranscriptions_CorrelationId`
...
- Multiple rows for the same `(VideoId, Language)` may share the same `CorrelationId` and `WorkerJobId` when one worker run produces both `VTT` and `SRT`
- `CorrelationId`, `WorkerJobId`, `Model`, and `FailureReason` make transcription orchestration durable from the backend side
- Live progress remains worker-polled rather than stored as a high-frequency database field
...
- In progress: VideoTranscriptions orchestration metadata, status APIs, and caption delivery
...
### v4.0 (June 20, 2026)
- Added durable transcription orchestration metadata to `VideoTranscriptions`
- Added `CorrelationId` and `WorkerJobId` for backend-to-worker job matching
- Added `Model` and `FailureReason` for provider observability and failure reporting
- Added worker/correlation indexes to support callback completion and status lookup
- Clarified caption delivery and canonical artifact ownership through Stream Forge storage
