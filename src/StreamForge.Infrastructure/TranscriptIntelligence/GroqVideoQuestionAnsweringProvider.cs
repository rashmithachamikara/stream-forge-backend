...
public sealed class GroqVideoQuestionAnsweringProvider : IVideoQuestionAnsweringProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new ArgumentException("A Groq model is required for question answering.", nameof(request));
        }
        ...
        var payload = new GroqChatCompletionsRequest(
            request.Model,
            [
                new GroqChatMessage("system", "You are a grounded transcript question answering assistant. Return JSON only."),
        ...
        var apiResponse = await response.Content.ReadFromJsonAsync<GroqChatCompletionsResponse>(_jsonOptions, cancellationToken)
            ?? throw new InvalidOperationException("Groq returned an empty response.");

        var rawText = ExtractText(apiResponse);
        var answerPayload = StructuredQuestionAnsweringResponseParser.Parse("Groq", rawText, request.Model, _logger);

        _logger.LogInformation(
            "Generated grounded answer through Groq model {Model} with {EvidenceCount} evidence chunk(s).",
            request.Model,
    ...
    private static string ExtractText(GroqChatCompletionsResponse response)
    {
        ...
        {
            throw new InvalidOperationException("Groq did not return structured text content.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        switch ((int)response.StatusCode)
        {
            case 429:
            {
                ...
                _logger.LogWarning(
                    "Groq rate limited request for model {Model}. RetryAfter: {RetryAfter}. Body preview: {BodyPreview}.",
                    model,
                    retryAfter,
                    preview);
            ...
            case 403:
                _logger.LogError(
                    "Groq authentication/authorization failure for model {Model}. Status: {StatusCode}. Body preview: {BodyPreview}.",
                    model,
            ...
            default:
                ...
                {
                    _logger.LogWarning(
                        "Groq upstream failure for model {Model}. Status: {StatusCode}. Body preview: {BodyPreview}.",
                        model,
                        (int)response.StatusCode,
                        preview);
                    throw new ExternalServiceException(
                ...
                _logger.LogWarning(
                    "Groq request failed for model {Model}. Status: {StatusCode}. Body preview: {BodyPreview}.",
                    model,
                    (int)response.StatusCode,
                    preview);
                throw new ExternalServiceException(
    ...
    private sealed record GroqChatCompletionsRequest(
        [property: JsonPropertyName("model")] string Model,
        [property: JsonPropertyName("messages")] IReadOnlyCollection<GroqChatMessage> Messages,
        [property: JsonPropertyName("temperature")] double Temperature,
        [property: JsonPropertyName("max_tokens")] int MaxTokens);

    private sealed record GroqChatMessage(
        [property: JsonPropertyName("role")] string Role,
        [property: JsonPropertyName("content")] string Content);

    private sealed record GroqChatCompletionsResponse(
        [property: JsonPropertyName("choices")] GroqChatChoice[]? Choices);

    private sealed record GroqChatChoice(
        [property: JsonPropertyName("message")] GroqChatChoiceMessage? Message);

    private sealed record GroqChatChoiceMessage(
        [property: JsonPropertyName("content")] string? Content);
}
